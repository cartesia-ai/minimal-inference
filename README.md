...
| 2 | 482ms | 14.8ms | 80 |
| 20 | 308ms | 18.7ms | 766 |
| 32 | 400ms | 21.4ms | 1,049 |

**TTFT vs T2T:** Time to First Token (TTFT) measures how long until the first generated token arrives — this includes the full prefill pass over the prompt. Token-to-Token (T2T) measures the average interval between subsequent decode tokens, excluding the first. These are reported separately because prefill is much more expensive than a single decode step and would inflate the average if combined. The benchmark script (`benchmark.py`) only counts inter-token intervals after the first token for T2T.

TTFT increases with batch size because prefill is sequential (one request at a time), so later requests queue behind earlier ones. T2T stays nearly flat because batched decode shares the GPU — going from 2 to 32 concurrent requests only adds ~7ms per decode step per stream while throughput scales ~13x.
